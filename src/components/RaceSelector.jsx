...
const RaceSelector = ({ handleRaceSelect }) => {
  ...
  return (
        <select 
          aria-label="Race Selector" 
          className="text-black border-amlime border-2 rounded-md m-4 h-8 cursor-pointer" 
          value={race} 
          onChange={e => {
            ...
            resetConstructor(); 
            resetShowPositions(); 
            resetShowDriverStandings(); 
            resetShowConstructorStandings(); 
            resetLoading();
            }
          }
        >
